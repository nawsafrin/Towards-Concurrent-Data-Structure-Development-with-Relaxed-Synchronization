  ...
  public static void main(String args[]) throws Exception {
    ...
    for (int iteration = 1; iteration <= NUM_OF_ITERATIONS; iteration++) {
      ...
      for (int i = 0; i < NUM_OF_THREADS; i++) {
        service.submit(new StackAddLock(list, NUM_OF_OPERATIONS, AStackConstants.SLEEP));
      } 
      for (int i = 0; i < NUM_OF_THREADS; i++) {
        service.submit(new StackRemoveLock(list, NUM_OF_OPERATIONS/2, AStackConstants.SLEEP));
      }
      service.shutdown();
