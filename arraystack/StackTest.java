...
public class StackTest {
  public static void main(String args[]) throws Exception {
    ...
    for (int iteration = 1; iteration <= NUM_OF_ITERATIONS; iteration++) {
      ...
      list = new StackDemo(size);    
      if (iteration == NUM_OF_ITERATIONS) {
        before = System.currentTimeMillis();    
      }
      for (int i = 0; i < NUM_OF_THREADS; i++) {
        service.submit(new StackAdd(list, NUM_OF_OPERATIONS, AStackConstants.SLEEP));
      } 
      for (int i = 0; i < NUM_OF_THREADS; i++) {
        service.submit(new StackRemove(list, NUM_OF_OPERATIONS/2, AStackConstants.SLEEP));
      }
      service.shutdown();
